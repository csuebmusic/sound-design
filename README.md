...
#### [`interference.html`](./interference.html)

Two sine waves at the same frequency with **independent amplitudes** A₁ and A₂, combined under a variable phase offset φ. The sum is `A₁·sin(θ) + A₂·sin(θ+φ) = R·sin(θ+ψ)` where `R = √(A₁² + A₂² + 2·A₁·A₂·cos(φ))` and `ψ = atan2(A₂·sin(φ), A₁ + A₂·cos(φ))`. The tool shows three things at once: a phasor addition diagram (two arrows tip-to-tail with the resultant in terra-cotta), a wave canvas with all three signals overlaid, and a live status badge that classifies the current phase as constructive (φ near 0° or 360°), partial interference, or destructive (φ near 180°). Key positions: at 0° amplitudes add fully (R = A₁ + A₂); at 90° they sum to R = √(A₁² + A₂²) — the same case shown in the phasor tool's complex panel; at 180° they cancel to R = |A₁ − A₂|, producing complete silence when A₁ = A₂ and otherwise leaving the difference. The tool is framed by a real-world scenario — two microphones on one source — with side-by-side schematic doodles showing wrong placement (d₂ ≈ d₁, comb filtering) and the 3:1 fix (d₂ ≥ 3·d₁, bleed too quiet to interfere). This connects abstract phase math to a problem students encounter in any recording session.
